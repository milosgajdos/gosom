...
func (m *Map) batchTrain(tc *TrainConfig, data *mat64.Dense, iters int) error {
	...
	for i := 0; i < iters; i++ {
		...
		wg := &sync.WaitGroup{}
		// Iterate over the input data in batchSize * workers
		for j := 0; j < rows; j += bSize * workers {
			...
			for k := 0; k < workers; k++ {
				...
				//fmt.Println("FROM", from, "COUNT", count)
				wg.Add(1)
				go m.processBatch(results, wg, bc, data, from, count, iter)
				// no need to spin another goroutine
				if count < bSize {
					break
				}
				iter++
...
func (m Map) processBatch(res chan<- *batchResult, wg *sync.WaitGroup,
	...
	//fmt.Println("batch FROM", from, "batch COUNT", count)
